...
FirebaseStore._applyFilter = (request, item) => {
	...
	ref.orderByKey().once("value").then(
		(dataSnapShot) => {
			...
			dataSnapShot.forEach(
				snapShot => {
          const payload = FirebaseStore._fromFirebase(request.resourceConfig, snapShot)
          if (FirebaseStore._applyFilter(request, payload)) {
            resultSet.push(payload)
          }
				}
      )
